...
<template>
    <div class="-mt-8 -mx-4 sm:-mx-6 lg:-mx-8">
        ...
        <div v-if="upcomingReservations.length > 0" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-4">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-900">{{ $t('dashboard.upcoming_reservations') }}</h2>
                <p class="text-sm text-gray-500 mt-1">{{ $t('dashboard.upcoming_reservations_subtitle') }}</p>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <a
                    ...
                    :key="res.id"
                    :href="`/portal/reservations/${res.id}`"
                    class="group bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-gray-100"
                >
                    <!-- Stay image or gradient -->
                    <div
                        class="h-32 flex items-center justify-center relative overflow-hidden"
                        :class="res.stay?.cover_image_url ? '' : 'bg-gradient-to-br from-indigo-400 to-violet-500'"
                    >
                        <img v-if="res.stay?.cover_image_url" :src="res.stay.cover_image_url" :alt="res.stay?.name" class="w-full h-full object-cover" />
                        <span v-else class="text-white/30 text-4xl font-bold">{{ res.stay?.name?.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2) || '?' }}</span>
                        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors"></div>
                        <span
                            class="absolute top-3 right-3 text-xs px-2.5 py-1 rounded-full font-semibold shadow-sm backdrop-blur-sm"
                            :class="res.status === 'confirmed' ? 'bg-blue-100/90 text-blue-700' : 'bg-amber-100/90 text-amber-700'"
                        >
                            {{ $t('status.' + res.status) }}
                        </span>
                    </div>
                    ...
                    <div class="p-4">
                        <h3 v-if="res.stay?.name" class="font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">
                            {{ res.stay.name }}
                        </h3>
                        <p v-if="res.stay?.address" class="text-xs text-gray-500 mt-1 flex items-center gap-1">
                        ...
                        <div class="flex items-center gap-2 mt-3 text-sm text-gray-600">
                            ...
                            <span>{{ res.period.check_in }} &rarr; {{ res.period.check_out }}</span>
                            <span class="text-xs text-gray-400">({{ res.period.nights }} {{ $t('reservation.nights') }})</span>
                        </div>
                    </div>
                </a>
            </div>
