...
<template>
    <div>
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800">{{ $t('stay.new') }}</h1>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-2xl">
            <form @submit.prevent="submit" class="p-6 space-y-5">
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    >
                        <span v-if="form.processing">{{ $t('stay.creating') }}</span>
                        <span v-else>{{ $t('stay.create') }}</span>
                    </button>
                    <a href="/stays" class="text-sm text-gray-500 hover:text-gray-700">{{ $t('common.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
