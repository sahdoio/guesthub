...
const form = useForm({
    ...
    contact_phone: props.hotel.contact_phone || '',
});

const submit = () => {
    form.put(`/hotels/${props.hotel.slug}`);
};
</script>
...
<template>
    <div>
        <div class="mb-6">
            <a :href="`/hotels/${hotel.slug}`" class="text-sm text-gray-500 hover:text-gray-700 mb-2 inline-block">&larr; {{ $t('common.back') }}</a>
            <h1 class="text-2xl font-bold text-gray-800">{{ $t('hotel.edit') }}</h1>
        </div>
        ...
        <div class="bg-white rounded-lg shadow max-w-2xl">
            <form @submit.prevent="submit" class="p-6 space-y-5">
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    >
                        <span v-if="form.processing">{{ $t('hotel.saving') }}</span>
                        <span v-else>{{ $t('hotel.save') }}</span>
                    </button>
                    <a :href="`/hotels/${hotel.slug}`" class="text-sm text-gray-500 hover:text-gray-700">{{ $t('common.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
